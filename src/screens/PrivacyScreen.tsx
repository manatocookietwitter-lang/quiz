  ...
  return (
    <Layout><div className="privacy-screen">
      ...
      <main className="privacy-screen__body">
        <p className="privacy-screen__updated">制定日：2026年8月5日／最終更新日：2026年8月24日</p>
        ...
        <section>
          <h2>3. ログインと共有機能</h2>
          <p>問題作成と学習にはログイン不要です。共有機能またはクラウド同期へログインする場合、メールアドレスをSupabase Authで処理し、ログイン用リンクを送信します。</p>
        ...
        <section>
          <h2>4. 任意のクラウド同期</h2>
          <p>クラウド同期は任意機能で、利用にはメールアドレスによるログインが必要です。利用者が同期機能を有効にして保存操作を行った場合だけ、問題データ、学習履歴、ノート、設定、ランダムに生成した同期ID、更新日時をSupabaseへ送信し、ログイン中のアカウントに関連付けて保護します。</p>
          <p>氏名、連絡先（ログイン用メールアドレスを除く）、正確な位置情報、広告識別子は取得しません。同期IDはクラウドデータを識別する秘密情報です。第三者へ共有しないでください。通信はHTTPSで暗号化されます。</p>
        </section>
        ...
        <section>
          ...
          <p>共有は各問題セットの「共有を停止」から削除できます。ログインアカウントと関連する共有・同期データは、設定画面の「ログインアカウントを削除」からまとめて削除できます。これらの操作で端末内の学習データは削除しません。第三者がすでに自分用に追加した独立コピーは、共有停止やアカウント削除後もその第三者の端末に残ることがあります。</p>
        </section>

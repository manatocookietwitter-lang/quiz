  ...
  if (screen.name === 'folder') {
    ...
    content = (
      <ProblemSetDetailScreen
        ...
        onOpenProblemList={({ category, sortMode }) => navigate({ name: 'problemList', setId: screen.setId, category, sortMode })}
    ...
    content = (
      <ProblemListScreen
        ...
        onChangeCategory={(category) => navigate({ name: 'problemList', setId: screen.setId, category, sortMode: screen.sortMode })}
        onStartFromQuestion={({ questions, initialIndex, title, subtitle, setId }) => handleStartQuizSession({
          title,
          subtitle,
          questions,
          mode: 'quiz',
          setId,
          initialIndex,
          backScreen: { name: 'problemList', setId: screen.setId, category: screen.category, sortMode: screen.sortMode },
        })}
      />
    );
...
function getScreenKey(screen: AppScreen) {
  ...
  if (screen.name === 'problemList') return `problem-list-${screen.setId}-${screen.category}-${screen.sortMode}`;

...
【問題の並び順】
questions の順番は学習順として重要です。ランダムや資料の抜粋順のままにしないでください。
1. まず category ごとにまとめる。
2. 同じ category 内では、近い内容・混同しやすい内容を連続して配置する。
3. 各 category 内は、学ぶべき順に並べる。
   - 定義・全体像
   - 分類・違い
   - 病態・機序
   - 検査・診断
...
【解説】
explanation は、正解の根拠が分かるように書いてください。
必要がある場合は、誤答選択肢についても「どこが違うのか」を短く触れてください。
ただし、全選択肢を機械的に長く解説しすぎないでください。
...
13. explanation は正解根拠を示し、必要に応じて誤答にも触れている。`;

...
3. すべての問題に category がある。
4. choices は4個または5個。
5. answerIndex / answerIndexes は0始まり。
6. answerIndex と answerIndexes が同時に入っていない。
7. reference に年度・問題番号が入っている。
8. 重複統合時は reference に複数年度・問題番号が併記されている。
9. questions が category ごとにまとまり、category 内も学習順に並んでいる。
10. explanation が短すぎず、正解理由が分かる。
11. explanation が長すぎず、不要な一般論を広げていない。
12. explanation が、次に似た過去問を解けるような内容になっている。`;

...
【資料の読み取り手順】
1. JSONを作る前に、入力されたすべてのPDF、画像、テキスト、表、図、脚注、注記を先頭から末尾まで確認する。一部のページやOCRの抜粋だけを見て作問しない。
2. 先に資料全体の見出し、章、節、問題番号、表・図の位置を内部的に一覧化してから、問題文と選択肢を作る。ページをまたぐ問題や続きの選択肢は、分割・重複させず一問として正しく結合する。
3. OCRは下書きとして使い、数字、単位、否定語、括弧、上付き・下付き、固有名詞、選択肢番号は原本で照合する。特に「ない」「以外」「誤り」などの否定、数値、単位、薬剤名、疾患名を優先して確認する。
4. OCRが欠落、文字化け、改行崩れ、段組みの混線を起こしている場合は、OCR結果から推測せず、ChatGPTがPDF・画像の該当ページを直接目視して読み取る。必要なら前後ページ、表、図、解答欄も確認する。
5. 原本で判読できない部分は創作・補完・推測をしない。問題文、選択肢、正答の根拠に関わる箇所は再確認し、確定できない内容を事実としてJSONに入れない。
6. 作問後に、資料全体の主要な定義、条件、例外、表・図の情報とquestionsを照合し、重要な内容の取りこぼしがないか確認する。
7. 資料に書かれていない知識を、資料にある内容のように追加しない。補足知識を入れる場合も、問題を解くために必要な範囲に限定する。
...
学習順の最終確認：
- category同士も、基礎・全体像から各論・応用へ進む順に並べる。分類名の五十音順、年度順、資料に現れた順だけで並べない。
- 同じcategory内では、同じ疾患・概念・検査・治療など近い内容を連続させ、関連する比較問題や混同しやすい問題を近くに置く。
- 原則として「定義・全体像 → 分類・違い → 病態・機序 → 検査・診断 → 治療・適応・禁忌 → 合併症・鑑別・例外 → 統合的な症例・識別問題」の順にする。
- 前提知識が必要な問題を、その前提を扱う問題より前に置かない。簡単な確認から中等度、応用・例外へ段階的に進める。
- 元資料の年度や問題番号はreferenceで保持し、学習順に並べ替えたことを理由に失わない。
...
分類の決め方：
- 先に資料全体から少数のcategory候補を作り、同じ基準で全問題に割り当てる。問題ごとに場当たり的な分類名を増やさない。
- categoryは、その問題が主に問う学習領域・概念で決める。年度、出題形式、難易度、参考ページ、治療・検査などの単独の属性をcategory名にしない。
- 問題セット全体では原則3〜10分類を目安にする。問題数が少なければ1〜3分類、大規模資料でも15分類を超えない。15を超えるのは、統合すると学習上不正確になる独立領域が明確に存在する場合だけにする。
- 表記ゆれ、同義語、略称は一つの代表名に統一する。意味が近い小分類は、学習上区別が必要でない限り上位のcategoryへ統合する。
- 1〜2問しかないcategoryは、独立した重要領域でない限り、内容が最も近い上位categoryへまとめる。
- 「未分類」は分類根拠が本当に不足する場合だけ使い、分類作業を省略するために使わない。
...
24. 資料の全ページ・表・図・脚注を確認し、重要な問題や条件の漏れがない。
25. categoryが増えすぎず、同じ主題の表記が統一され、未分類が便宜的に使われていない。
26. category同士とcategory内のquestionsが、基礎から応用へ学習しやすい順に並んでいる。
`;
...
【過去問資料の確認手順】
1. 年度、試験回、ファイルごとに、すべてのページを先頭から末尾まで確認する。問題ページだけでなく、解答、正答表、脚注、別紙、続きのページも確認する。
2. JSON作成前に、各資料について「年度・回・問題番号・掲載ページ・続きの有無」の内部チェックリストを作る。問題番号が飛んでいる場合は、未掲載なのか読み落としなのかを確認する。
3. 二段組み、ページ跨ぎ、表の中の選択肢、画像化された文字、縦書き、ヘッダー・フッターの混入を確認し、問題文と選択肢の対応を崩さない。
4. OCRの誤読が疑われる数字、単位、否定語、順位、薬剤名、疾患名、選択肢番号は、必ずPDF・画像の原本と照合する。OCRにない語を前後の文脈だけで補わない。
5. 解答欄や正答表がある場合は、問題番号と照合してanswerIndex・answerIndexesを確定する。正答の根拠が確認できない場合は推測で埋めず、原本と関連資料を再確認する。
6. 一つの過去問が複数ページに続く場合は一問に結合し、別ページの問題を前の問題に誤結合しない。年度や問題番号が変わった箇所は新しい問題として扱う。
7. すべての資料内問題を、出力問題そのものまたは統合先のreferenceに対応づける。対応づけられない問題を、重複を理由に黙って削除しない。
8. 問題文・選択肢・正答・referenceのいずれかが判読不能な場合は、推測で過去問を再構成しない。原本を直接確認してから出力する。
...
分類の決め方：
- 過去問の年度や試験回ではなく、問題が主に問う知識領域をcategoryにする。年度、問題番号、出題形式、難易度をcategory名にしない。
- 先に問題セット全体の少数の分類表を作り、同じ疾患・概念・制度・テーマは同じcategoryへ統一する。表記ゆれや略称を増やさない。
- 問題セット全体では原則3〜10分類を目安にし、大規模資料でも15分類を超えない。細分化すると学習のまとまりが失われる場合は、より広い上位categoryへまとめる。
- 1〜2問だけのcategoryは、独立した重要領域でない限り、最も近い上位categoryへ統合する。重複統合した問題は、統合後の主題に合わせて一つのcategoryにする。
- 「未分類」は原本から主題を判断できない場合だけ使い、年度や問題形式を理由に未分類へ逃がさない。
...
9. 内部チェックリストの各項目に、出力されたquestionのidとreference、または統合先のquestion idを対応づける。
10. 同じ年度・問題番号が複数回現れる場合は、原本の重複か別問題かを確認し、重複ならreferenceを統合し、別問題なら両方残す。
11. 資料の最初の問題から最後の問題まで、問題番号の連続性とページ範囲を最後に再確認する。見つからない番号を黙って飛ばさない。
12. 問題数を単純に一致させる必要がない統合の場合でも、削除されたのではなく、どの問題に統合されたかをreferenceで追跡できるようにする。
【問題の並び順】
...
22. 資料の全ページ・年度・問題番号・解答表を確認し、すべての問題が出力または統合先referenceから追跡できる。
23. categoryが増えすぎず、同じ主題の表記が統一され、未分類が便宜的に使われていない。
24. category同士とcategory内のquestionsが、基礎から応用へ学習しやすい順に並んでいる。
`;
